...
const STEPS = [
  ...
  },
  {
    ...
    description: "Create Brand Identities to switch between different voices, tones, and audiences instantly.",
    icon: User,
  ...
  },
  {
    ...
    description: "Generate high-conversion posts for LinkedIn, Twitter, and Threads using simple natural language.",
    icon: Layout,
  ...
  },
  {
    ...
    description: "Save your favorite generations to your personal library for future use and repurposing.",
    icon: Library,
...
export default function FeatureWalkthrough() {
  ...
  useEffect(() => {
    ...
    const key = `ghostwrites_onboarding_seen_${user.id}`;
  ...
  const handleClose = () => {
    if (user?.id) {
      localStorage.setItem(`ghostwrites_onboarding_seen_${user.id}`, 'true');
    }
    setIsOpen(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        <div className={`absolute -top-16 -left-16 w-48 h-48 bg-gradient-to-br ${step.glow} blur-[80px] rounded-full pointer-events-none transition-all duration-700`} />
        <div className={`absolute -bottom-16 -right-16 w-48 h-48 bg-gradient-to-br ${step.glow} blur-[80px] rounded-full pointer-events-none transition-all duration-700`} />
        ...
        <div className="relative z-10 p-6 sm:p-8">
          ...
          <div className="flex justify-center mb-6">
            <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl ${step.bg} border ${step.border} flex items-center justify-center shadow-lg`}>
            ...
            {STEPS.map((_, i) => (
              <div
                ...
                className={`h-1 rounded-full transition-all duration-500 ${
                  i === currentStep ? 'w-6 bg-white' : i < currentStep ? 'w-3 bg-white/40' : 'w-3 bg-white/10'
                }`}
              />
            ))}
          </div>
